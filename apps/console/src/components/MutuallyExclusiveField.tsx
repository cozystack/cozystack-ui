...
interface MutuallyExclusiveFieldProps {
  schema: RJSFSchema
  formData: Record<string, unknown> | undefined
  handleChange: (data: Record<string, unknown> | undefined) => void
  title?: string
  description?: string
  required?: boolean
}
...
export function MutuallyExclusiveField(props: MutuallyExclusiveFieldProps) {
  ...
  const currentOption = formData
    ? options.find((opt) => formData[opt] !== undefined && formData[opt] !== null)
    : undefined
    ...
    const defaultValue =
      prop && typeof prop === "object" && Object.keys(prop.properties || {}).length === 0
        ...
        : prop && typeof prop === "object" && prop.type === "object"
          ? {}
          : ""
    handleChange({ [option]: defaultValue })
  }
  ...
  return (
    <div className="field">
      ...
      {description && <p className="field-description mb-3">{description}</p>}
